...
function createCard(div) {
  var title = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : "Untitled Place";
  var imageUrl = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : "https://icon-library.com/images/no-image-icon/no-image-icon-0.jpg";
...
var render = function render(array) {
  cards.innerHTML = '';
  array.forEach(function (house) {
    (0, _createCard.default)(cards, house.name, house.img, house.desc);
  });
...
var filterCard = document.querySelector('.select');
filterCard.addEventListener('click', function () {
  var value = filterCard.value;

  if (value === 'All') {
    render(_houseToRent.default);
  } else {
    var houses = _houseToRent.default.filter(function (house) {
...
var searchCard = document.getElementById('searchBlabla');
searchCard.addEventListener('input', function () {
  var wordSearch = searchCard.value.toLowerCase();

  var houses = _houseToRent.default.filter(function (poulet) {
    return poulet.name.toLowerCase().includes(wordSearch);
  });

  render(houses);
}); // On change (Show only available)

var isAvailable = document.querySelector('.available-checkbox');
isAvailable.addEventListener('click', function () {
  var houseAvailable = isAvailable.checked;

  if (houseAvailable) {
    render(_houseToRent.default.filter(function (house) {
      return house.available;
    }));

    if (_houseToRent.default.every(function (element) {
...
render(_houseToRent.default);
},{"./houseToRent":"src/houseToRent.js","./createCard":"src/createCard.js"}],"../../../../../../../usr/local/lib/node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
